'''
The module is designed for classical Monte Carlo simulation on Ising type lattice spin models.
'''
...
class hamiltonian():
    ...
    def neighborenergy(self, pos, configs, divided=0):
        '''
        ...
        :param configs: the array with the system sizelist shape
        ...
        for nearest in range(0, self.maxsep+1):
            ...
            nbtotal = 0
            nbtotal = sum([configs[tuple(nbpos)] for nbpos in nb])
        ...
        return np.inner(coupling, spinsum)*configs[tuple(pos)]

    def totalenergy(self, configs):
        '''
        ...
        '''
        te = 0
        for pos in np.ndindex(configs.shape):
            te += self.neighborenergy(list(pos), configs, divided=1)
        return te/2
...
class configuration():
    ...
    def mcmove(self, beta, autotime, method='local_update', tool=[0, 0, 0]):
        ...
        if method == 'local_update':
            ...
            self.rbm_update(autotime, tool[0], tool[1], tool[2])
    ...
    def rbm_update(self, autotime, rbmobj, nosteps, binary):
        '''
    ...
    def simulate(self, beta, num=9, autotime=1, pretime=1, method='local_update'):
        '''
